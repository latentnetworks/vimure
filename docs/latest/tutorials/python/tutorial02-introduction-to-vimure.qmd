...
(_If you rather see what the model is doing, you can set `verbose=True` in the `fit()` method_)
...
Node `113302` does not report any reciprocal ties, but it does report ties with six other nodes.
...
That is why `113302`'s $\theta\lambda_{k=1}$ is high: it reports a lot of ties, but only one of them is confirmed by other reporters.
