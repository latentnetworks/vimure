...
There is more to talk about how reporters are internally represented in VIMuRe. Read more about it on section [3.3: A note on the reporter mask (`R`)](#a-note-on-the-reporter-mask-r).
...
In the subsections below, we will provide an explanation of $\nu$ and $\theta_{lm}$ parameters and how to reason about mutuality and "reliability" of reporters. $\rho$ and $\lambda$ are more technical and we will go into detail about them in a future tutorial. 
...
The code above will return a $N \times N$ weighted adjacency matrix which can be thought of as the probability of a tie between individuals $i$ and $j$. More on that in the next tutorial.
...
We only have one layer, so the first dimension is 1. The second dimension is the number of reporters in the network. In our case, this shows that there are 324 reporters in the network. 
...
This is akin to saying each node has _the potential_ to be a reporter. But to control for which nodes are actually reporters, we use a binary mask, $R_{lijm}$, which is 1 if the node $m$ is a reporter on layer $l$ for the tie between nodes $i$ and $j$ and 0 otherwise. You can control how this mask is built by passing the `R` parameter to the `fit()` method. Read more about it on section [3.3: A note on the reporter mask (`R`)](#a-note-on-the-reporter-mask-r).

Changing this is a priority for the next version of the package. But, as it is not a trivial change right now, we have decided to leave it as it is for now.
...
The interpretation depends on both the mutuality of the network and the survey design. If the mutuality is high (as indicated by $\nu$ being closer to 1), reporters will tend to reciprocate the ties they report by default, and that reporters with high $\theta$ values (relative to the $\theta$ distribution) are likely "over-reporting" ties.

To verify if this is the case for the network, let's examine the top reporter listed in the table above (113901).

```python
edgelist[edgelist["reporter"] == "113901"]
```

|    ego |   alter |   reporter | tie_type    | layer   |   weight |
|-------:|--------:|-----------:|:------------|:--------|---------:|
| 113901 |  **110201** |     113901 | lendmoney   | money   |        1 |
| 113901 |  **114702** |     113901 | lendmoney   | money   |        1 |
| **112901** |  113901 |     113901 | borrowmoney | money   |        1 |
| **117301** |  113901 |     113901 | borrowmoney | money   |        1 |
| **114101** |  113901 |     113901 | borrowmoney | money   |        1 |
...
To avoid making inaccurate assumptions, it's important to refrain from categorically stating that a reporter with $\theta_m = 0$ is "under-reporting" ties. This is because the survey design can also impact how we interpret the results. For instance, in the Karnataka study, the name generators could only report a maximum of four ties per prompt. Therefore, in some cases, $\theta_m = 0$ might simply indicate that the reporter has reported all their available ties rather than deliberately withholding information.
...
Amonst the nodes reported, only `110502` is a reporter and it does not report any ties with `114802` (run `edgelist[edgelist["reporter"] == "110502"]` to verify this).
