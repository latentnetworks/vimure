...
def get_layer(village_id, layer_name, indivinfo, 
    ...
    for tie_type in selected_tie_types:
        edgelist_, respondents_ = get_karnataka_survey_data(village_id=village_id, 
                                                            tie_type=tie_type, 
                                                            indivinfo=indivinfo,
                                                            raw_csv_folder=raw_csv_folder)
        edgelist = pd.concat([edgelist, edgelist_])
        respondents = respondents.union(respondents_)

    return edgelist, respondents


indivinfo = pd.read_stata("docs/data/datav4.0/Data/2. Demographics and Outcomes/individual_characteristics.dta")
indivinfo.drop_duplicates(subset=["pid"], inplace=True) ## one individual (6109803) is repeated twice.
indivinfo["pid"] = indivinfo["pid"].astype(str)
indivinfo["hhid"] = indivinfo["hhid"].astype(str)
...
for i in VALID_VILLAGE_IDS:
    for layer_name in ["money", "advice", "kerorice", "visit"]:
      ...
      edgelist, respondents = get_layer(village_id=i, 
                                        layer_name=layer_name, 
                                        indivinfo=indivinfo, 
                                        raw_csv_folder=RAW_CSV_FOLDER)
      edgelist.to_csv(f"vil{i}_{layer_name}.csv", index=False)
      # save respondents to a separate file
      pd.DataFrame({"pid": list(respondents)}).to_csv(f"vil{i}_{layer_name}_respondents.csv", index=False)
