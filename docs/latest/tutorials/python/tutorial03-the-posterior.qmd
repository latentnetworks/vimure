...
The plots above give us an idea of how sparse the network is, but without any meaningful node ordering, it's hard to see its structure. In the next section, we'll use $\rho_{lijk=2}$ as a measure of edge strength and treat it as a point estimate. This will help us get a clearer picture of this multi-layered network.

## 🎲 Step 3: Obtaining a simple point estimate



## 📋 Step 4: Compare different models using information criteria
...
We can also compare models with different settings or priors using these criteria. For example, we can fit another model with a different prior mean for θ.
