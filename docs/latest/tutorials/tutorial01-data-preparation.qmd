...
The survey that produced this data collected information on 5 types of relationships:
...
The data is in a non-standard format and so it will need to be further pre-processed as an edge list:
...
Each row represents reports made by a single individual; the first number is the `pid` of the individual who reported the relationship, whereas the remaining however many numbers listed in the same row are the `pid`s of the individuals who were reported to be involved in the relationship.
...
for (i in VALID_VILLAGE_IDS){
  ...
  for(selected_tie_type in tietypes){
  ...
  write.csv(edges, name = paste0("vil",i,"_edges.csv"))
  write.csv(indiv, name = paste0("vil",i,"_meta.csv"))
...
[^1]: Note that the authors provide a different version of the edge list data on Harvard Dataverse [@banerjee_diffusion_2013-1]. However, we will use the raw version provided by Prof. Jackson in this tutorial, as this was the version used in our paper. We will use the Harvard Dataverse files just for the metadata.