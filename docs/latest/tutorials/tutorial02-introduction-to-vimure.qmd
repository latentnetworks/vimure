...
If we had a different survey setup, we would need to specify the `R` parameter in the `parse_graph_from_edgelist()` function. Read more on the [💻 **Tutorial 03**: Handling multiple survey setups](/docs/latest/tutorials/tutorial03-different-survey-setups.qmd) tutorial.
:::
...
<mark style="color:red">Do not revise beyond this point. The plots of inferred networks do not look like I expected, and I am trying to figure out why.</mark>
...
G_rho <- igraph::graph_from_adjacency_matrix(Y_hat, mode = "directed", weighted=TRUE) 

c_scale <- colorRamp(c('#ffffff', '#b4c1d3'))

E(G_rho)$color = apply(c_scale(E(G_rho)$weight), 1, function(x) rgb(x[1]/255,x[2]/255,x[3]/255) )
...
G_rho_max <- igraph::graph_from_adjacency_matrix(Y_hat[1, , ], mode = "directed", weighted=TRUE) 
```
...
pos <- layout_with_fr(G_union, niter=200)
```
...
# plot(G_rho, layout=pos, main=expression(paste("Inferred network (", hat(rho), ")")))
