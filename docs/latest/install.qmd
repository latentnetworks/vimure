...
2. Set up a python environment for reticulate so it doesn't mess up other python installations you might have installed on your computer.

    ```r
    reticulate::conda_create("venv-vimure")
    reticulate::use_condaenv("venv-vimure", required = TRUE)
    ```

3. Close the R session and open it again to make sure the changes take effect.

4.  Check if `reticulate` has a valid Python installation by running the following command:
    ...
    ```r
    python:         ..path../anaconda3/envs/venv-vimure/bin/python
    libpython:      ..path../anaconda3/envs/venv-vimure/lib/libpython3.8.so
    pythonhome:     ..path../anaconda3/envs/venv-vimure:..path../anaconda3/envs/venv-vimure
...
5. (Advanced) Alternatively, you can set up `reticulate` to use other pre-installed Python versions you might have on your computer. See [Alternative Versions section](src/R/README.md#alternate-versions-advanced-mode) on our R README for more details.
